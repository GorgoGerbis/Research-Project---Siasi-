...
######################### New Terminal Stuff ####################################
#@ ToDo REMEMBER TO ADJUST FINAL FAILURE AVERAGES TO NOT COUNT TERMINALS
...
def create_link_node_terminal_duos(num_terminals, num_nodes, max_links_per_terminal_node):
    existing_links = set()
    existing_terminal_links = set()
    available_nodes = ["A", "B", "C", "D", "E"] # [x for x in range(1, num_nodes + 1)]
    ...
    while count < num_terminals:     # len(existing_links) < num_links OR len(available_nodes) > 0
        source = random.choice(available_terminals)
        destination = random.choice(available_nodes)
        link_coords = (source, destination)

        if (link_coords not in existing_terminal_links) and (source != destination) and already_added.count(destination) <= max_links_per_terminal_node:
    ...
    #     link_coords = (source, destination)
    #
    #     if (link_coords not in existing_links) and (source != destination) and (already_added.count(source) < 4 and already_added.count(destination) <= 4):
    #         existing_links.add(link_coords)
    #         already_added.append(source)
    #         already_added.append(destination)
    #         count += 1
...
def create_node_and_link_input_data(num_links, num_terminals, num_nodes):
    existing_links = create_link_duos(num_nodes, 4)
...
if __name__ == '__main__':
    ...
    # create_node_and_link_input_data(NUM_LINKS, NUM_TERMINALS, NUM_NODES)
    #
    # # createRequests(NUM_REQS, NUM_NODES)
    # create_terminal_requests(NUM_REQS, NUM_TERMINALS)
    #
    # print("FINISHED CREATING INPUT DATA\n")
    # create_link_node_terminal_duos(NUM_TERMINALS, NUM_NODES, NUM_LINKS)
    create_link_node_terminal_duos(15, 5, 35)
