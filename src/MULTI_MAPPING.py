...
def calculate_path_resources(path_obj, req_vnfs):  # <-- MULTI-MAPPING
    ...
    can_map_all_vnfs, mapping_locations = path_obj.determine_path_node_resources_MULTI(req_vnfs)
...
def map_path_MULTI(path_obj, req_bw):
...
def RUN_PATH_ONE(req):
    ...
    if len(PathObj.BACKUP_PATHS) == 0:
        ...
        map_path_MULTI(optimal_path, req_bw)
